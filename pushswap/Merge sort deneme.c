...
// Yığındaki medyanı bulan yardımcı fonksiyon
int find_median(Stack *stack)
{
    ...
    // temp dizisini sıralayarak medyanı bul
...
{
    // a yığını sıralanana kadar elemanları b yığınına it
    while (a->top > 0)
    {
    ...
    // b yığınındaki elemanları a yığınına geri taşı
    while (!is_empty(b))
    {
        pa(a, b);
    }
